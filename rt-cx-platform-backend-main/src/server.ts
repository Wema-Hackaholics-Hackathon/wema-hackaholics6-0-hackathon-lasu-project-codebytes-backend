...
async function startServer() {
  try {
    ...
    logger.info("⚠️ Database skipped for development");

    // Test Redis connection
    // await redis.ping();
    logger.info("⚠️ Redis skipped for development");
    ...
    const shutdown = async (signal: string) => {
      ...
      server.close(async () => {
        ...
        logger.info("Database disconnected (skipped)");

        // Close Redis connection
        // await redis.quit();
        logger.info("Redis disconnected (skipped)");
